...
新しいマシンに環境を作るところは、問題の簡単な方の半分でしかありません。
難しいのは、仕事用、プライベート用、ホスト、開発用 VPS、使い捨ての検証環境といった複数のマシンを、数か月後も信用できる状態のまま保つことです。
どこに何を入れたか、何が古くなっているか、他の環境と差分がないか。
これを人間が覚えておかなければならなくなると、更新作業そのものは難しくなくても、思い出して確認するコストの方が先に折れて放置されます。

そのため、次の 4 つを重視しています。

- **初回セットアップだけでなく、継続的な維持**。最初の導入を自動化するのは半分にすぎず、すでにある環境を何年も維持し続けられることの方が本題です。
- **記憶に依存しない更新**。更新があるかどうかは、思い出すのではなく検出します。マシンが知らせ、人間はいつ適用するかだけを決めます。
- **環境間の再現性**。各マシンが個別に upstream を追いかけることはしません。バージョンは中央で一度だけ解決し、各マシンは Git と lockfile を通してその結果へ追従します。更新の時期が 1 週間ずれた 2 台でも、行き着く状態は同じです。
- **更新しやすさと、無条件に信用しないことの両立**。更新頻度を上げた結果、新しい upstream release をそのまま信用する状態になっては意味がありません。追従の速さと慎重さは、独立した 2 つの目標ではなく 1 つのトレードオフです。
...
`install.sh` と `/dotfiles apply` がこれを移行するのは、ローカルで何も足されていないと証明できるときだけです（配置しようとしているコピーと同一か、インストール元リビジョンの `.mise.toml` と同一か）。
それ以外はファイルをその場に残すので、ピン留めは効いたままです。そのうえで次にすべきこと（中身を確認して端末固有の部分だけを残す、または移行を強行する `DOTFILES_MIGRATE_MISE_CONFIG=1`）を表示します。
...
中央で解決したバージョンに各マシンが追従するこの構造は、そのままサプライチェーン対策も兼ねています。
